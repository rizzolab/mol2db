...
def make_kwargs(args):
    kwargs = {}
    kwargs = vars(args)
    if (all(v is None for v in [args.dbname,\
                                args.user_name,\
                                args.ht, \
                                args.prt])):
        return kwargs
...
def set_configure (args):
    ...
    #update the config file
        ...
        sys.exit()
    elif (kwargs['subcommand'] in ["updatesource","createsource"] and \
          if_any_exist(**kwargs) == False):
    ...
    elif (kwargs['subcommand'] != "updatesource" and kwargs['subcommand'] != "createsource"):
